...
def show_verbal():
    ...
    ans1 = st.radio("Synonym for TEPID",{"","hot","warm","cold","boiling"})
    score= score+1 if ans1=="warm" else score

    ans2 = st.radio("Antoym for CONCEDE",{"","object","refuse","grant","accedes"})
    score= score+1 if ans2=="refuse" else score

    ans3 = st.radio("'To hit the nail right on the head' means",{"","To do the right thing","To destroy one's reputation","To announce one's fixed views","To teach someone a lesson"})
    score= score+1 if ans3=="To do the right thing" else score

    ans4 = st.radio("The waiter hasn't brought the coffee _____. I've been here an hour already.",{"","till","up","yet","still"})
    score= score+1 if ans4=="yet" else score

    ans5 = st.radio("Tractor : Trailer :: Horse : ?",{"","stable","cart","engine","saddle"})
    ...
    ans6 = st.radio("In his dream Harold found the loathsome creatures _____",{"","in his village","in his own house","in a different land","in his office"})
    ...
    ans7 = st.radio("Which of the following should be the second sentence after rearrangement?",{"","C","F","B","D","E"})
    ...
    ans8 = st.radio("Which of the following can be inferred from the above statement?",{""," Increase in the frequency of the trains would lead to decrease in number of such accidents","The trains in the Mumbai suburban run overcrowded ","The railway authorities are in different to the safety of commuters","The railway did not increase the frequency in proportion to the increase in the number of commuters in the past one year","None of these"})
    ...
    ans9 = st.radio("",{"","Only conclusion (I) follows","Only conclusion (II) follows ","Both conclusion follow","Neither conclusion (I) nor conclusion (II) follows ","None of these"})
    ...
    ans10 = st.radio("Despite his best efforts to conceal his anger _____",{"","we could detect that he was very happy","he failed to give us an impression of his agony","he succeeded in camouflaging his emotions","he could succeed in doing it easily","people came to know that he was annoyed"})
