...
def show_coding_test():
    ...
    st.image(image)
    ans3 = st.radio("",{""," loop loop find loop","Infinite error","find loop","Compile time loop"})
    score = score+1 if ans3=="find loop" else score

    st.write("4. Out of the following function definition, which one will run correctly?")
